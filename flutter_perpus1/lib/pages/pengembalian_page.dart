...
class _PengembalianPageState extends State<PengembalianPage> {
  ...
  Future<void> _loadPeminjaman() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost/perpustakaan/api/get_peminjaman.php'),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: const Color.fromARGB(255, 96, 228, 251),
                    ...
                    Row(
                      children: [
                        ClipRRect(
                          ...
                          child: Image.network(
                            peminjaman['image'] ?? 'https://via.placeholder.com/100',
                            width: 80,
                            height: 120,
                            fit: BoxFit.cover,
                          ),
                        ...
                        IconButton(
                          ...
                          onPressed: () async {
                            await _kembalikanBuku(peminjaman);
                          },
                        ),
                      ],
                    ),
  ...
  Future<void> _kembalikanBuku(Map<String, dynamic> peminjaman) async {
    try {
      final response = await http.post(
        Uri.parse('http://localhost/perpustakaan/api/return_book.php'),
        body: {
